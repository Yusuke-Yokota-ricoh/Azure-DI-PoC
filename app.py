...
if uploaded:
    ...
    with st.spinner("Azure Document Intelligence / Azure OpenAI で分析中（2スレッド並列実行）..."):
        try:
            def _layout_then_gpt(pdf_bytes: bytes):
                _lay = analyze_layout_regex(pdf_bytes)
                _lay_gpt = analyze_layout_gpt(_lay, pdf_bytes)
                return _lay, _lay_gpt

            with ThreadPoolExecutor(max_workers=2) as executor:
                f_lay = executor.submit(_layout_then_gpt,   pdf_bytes)
                f_vis = executor.submit(analyze_gpt_vision, pdf_bytes)
                lay, lay_gpt = f_lay.result()
